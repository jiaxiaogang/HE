...
+(void) recognitionFeatureV2_Step2:(AIFeatureJvBuModels*)decoratorJvBuModel protoColorDic:(NSDictionary*)protoColorDic ds:(NSString*)ds {
    ...
    NSArray *validModels = [SMGUtils sortBig2Small:decoratorJvBuModel.stModels compareBlock:^double(AIFeatureJvBuModel *obj) {
        ...
        return obj.matchValue * obj.modelMatchCountScore * obj.adjacentScore * obj.centerScore;
    }];
    ...
    for (AIFeatureJvBuModel *model in decoratorJvBuModel.stModels) {
        ...
        NSLog(@"%ld. 单特征识别结果:T%ld \t(%ld/%ld) \t匹配度:%.2f \t匹配数防抽:%.2f \t相邻度:%.2f \t中心度:%.2f = 总分:%.2f",
              [decoratorJvBuModel.stModels indexOfObject:model],model.assT.pId,model.bestGVs.count,model.assT.count,
              model.matchValue,model.modelMatchCountScore,model.adjacentScore,model.centerScore,
              model.matchValue * model.modelMatchCountScore * model.adjacentScore * model.centerScore);
        [SMGUtils runByMainQueue:^{
    ...
    [TCRecognitionInvoke printLogDescRate:decoratorJvBuModel.stModels protoLogDesc:nil prefix:@"单特征" convertNodeBlock:^id(AIFeatureJvBuModel *obj) {
        ...
        return obj.matchValue * obj.modelMatchCountScore * obj.areaRankRatio * obj.adjacentScore;
    }];
}
