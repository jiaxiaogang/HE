...
+(NSArray*) recognitionGroupFeatureV6:(AIKVPointer*)protoFeature_p matchModels:(NSArray*)matchModels {
    ...
    for (AIFeatureJvBuModel *obj in matchModels) {
        ...
        // 分析1、所有conST对撞（取交）不上：即conST没重复过，只有这一条。
        // 分析2、所有assST都对撞（取交）不上：即assST也都是conST而不是assST，它也没被GT引用过（为了方便调试，可以把抽象次数absLevel打出来）。
        // 思路、说白了，这个问题的根本，在于快速的st抽象，只要st可以快速抽象，它就能更快构建和识别到可以取交对撞上的gt结果。
