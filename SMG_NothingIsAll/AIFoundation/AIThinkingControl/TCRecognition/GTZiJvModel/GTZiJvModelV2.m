...
-(CGFloat) zonHeScore {
    ...
    return self.gtOuterShapeMatchValue * self.gtInnerEigenMatchValue * [AIScore tailScore:self.matchCountRatioV2] * [AIScore tailScore:self.bestsCountScoreByRank] * [AIScore tailScore:self.averageContentStrongScore];
}
...
-(NSString*) zonHeDesc {
    ...
    return STRFORMAT(@"外形:%.2f 内征:%.2f 匹配率:%.2f (%02ld/%02ld) 匹配数:%.2f 稳定性:%.2f = 总分:%.3f",self.gtOuterShapeMatchValue,self.gtInnerEigenMatchValue,
                     [AIScore tailScore:self.matchCountRatioV2],self.bestSTs.count,self.baseGT.count,
