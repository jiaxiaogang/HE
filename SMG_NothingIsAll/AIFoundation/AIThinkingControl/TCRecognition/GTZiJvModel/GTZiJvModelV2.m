...
-(CGFloat) zonHeScore {
    ...
    return self.gtOuterShapeMatchValue * self.gtInnerEigenMatchValue * self.matchCountRatioV2 * self.bestsCountScoreByRank * self.averageContentStrongScore;
}
...
-(NSString*) zonHeDesc {
    ...
    // v5: 随着加权求和切图法上线，匹配数和匹配率全是100%，所以改回只用匹配度。
    return STRFORMAT(@"外形:%.2f 内征:%.2f 匹配率:%.2f (%02ld/%02ld) 匹配数:%.2f 稳定性:%.2f = 总分:%.3f",self.gtOuterShapeMatchValue,self.gtInnerEigenMatchValue,self.matchCountRatioV2,self.bestSTs.count,self.baseGT.count,self.bestsCountScoreByRank,self.averageContentStrongScore,self.zonHeScore);
}
